  ...
  describe('deployment', async () => {
    ...
    describe('with a positive balance', async () => {
      ...
      it('transfers', async () => {
        ...
        expect(await erc20.balanceOf(user1)).to.be.equal(fromBalanceBefore.sub(1))
        expect(await erc20.balanceOf(user2)).to.be.equal(toBalanceBefore.add(1))
      })
      ...
      it('should not transfer beyond balance', async () => {
        await expect(erc20.connect(user1Acc).transfer(user2, 100)).to.be.revertedWith('ERC20: Insufficient balance')
        await expect(erc20.connect(user1Acc).transferFrom(user1, user2, 100)).to.be.revertedWith('ERC20: Insufficient balance')
      })
      ...
      it('approves to increase allowance', async () => {
        ...
        expect(await erc20.allowance(user1, user2)).to.be.eq(allowanceBefore.add(1))
      })
      ...
      describe('with a positive allowance', async () => {
        ...
        it('should not transfer beyond allowance', async () => {
          await expect(erc20.connect(user2Acc).transferFrom(user1, user2, 20)).to.be.revertedWith('ERC20: Insufficient approval')
        })
      })
